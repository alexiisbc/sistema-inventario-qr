...
# --- RUTAS DE VISTAS (IGUAL QUE ANTES) ---
...
def index():
    ...
    stats = {'total_productos': total_prods, 'criticos': criticos, 'valor_total': "{:,.0f}".format(valor_total).replace(',', '.')}
...
def add_product():
    ...
    try:
        ...
        flash('Error: SKU ya existe.', 'danger')
    return redirect(url_for('index'))
